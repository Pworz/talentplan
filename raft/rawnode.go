...
func (rn *RawNode) Ready() Ready {
	...
	rd := Ready{
		SoftState: nil,
		// 获取raftLog的unstable部分存储的Entry
		// 交给上层模块持久化
		Entries: rn.Raft.RaftLog.unstableEntries(),
		// 获取已提交未应用的Entry,从applied到committed
		CommittedEntries: rn.Raft.RaftLog.nextEnts(),
		Messages:         rn.Raft.msgs,
	}
	// 清空消息
	rn.Raft.msgs = nil
	// 检测两次创建Ready时,raft的状态是否发生了变化
	// 若发生变化,则返回变化的部分
	// 否则返回空(相关字段为nil,表示无需上层处理)
	hardState := rn.HardState()
	softState := rn.SoftState()
	if !isHardStateEqual(rn.prevHardSt, hardState) {
		rd.HardState = hardState
	}
	if !isSoftStateEqual(*rn.prevSoftSt, *softState) {
...
func (rn *RawNode) HasReady() bool {
	// Your Code Here (2A).
	// 仿etcd containsUpdates(), 检测Ready各个字段是否为空
	// 若有变化,则返回true, 否则返回false
	// 决定是否需要交由上层模块处理
	hardState := rn.HardState()
	softState := *rn.SoftState()
	// 是否有需要持久化的Entry,
	if len(rn.Raft.RaftLog.unstableEntries()) > 0 ||
		...
		// raft状态是否发生变化,
		(!IsEmptyHardState(hardState) && !isHardStateEqual(rn.prevHardSt, hardState)) ||
		!isSoftStateEqual(*rn.prevSoftSt, softState) {
		return true
	}
	return false
}
