	...
	if len(rd.CommittedEntries) > 0 {
		...
		for _, entry := range rd.CommittedEntries {
			...
			if entry.EntryType == eraftpb.EntryType_EntryConfChange {
				// 使用现有的处理方法
				d.HandleMsg(message.Msg{Type: message.MsgTypeRaftCmd, Data: &message.MsgRaftCmd{
					Request:  &raft_cmdpb.RaftCmdRequest{AdminRequest: &raft_cmdpb.AdminRequest{CmdType: raft_cmdpb.AdminCmdType_ChangePeer}},
					Callback: nil,
				}})
			} else {
				// 使用现有的处理方法
...
// 处理提议回调
func (d *peerMsgHandler) processProposals(entry *eraftpb.Entry) {
	// 找到对应的提议
